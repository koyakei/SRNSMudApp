...
public class TaggingRequestRejectE2ETests
{
    ...
    public async Task TaggingRequest_ShouldBeRejectableByItemOwner()
    {
        ...
        IPage user2Page = await context.NewPageAsync();
        await user2Page.GotoAsync($"{_factory.ServerAddress}/auth/callback?provider=Google&code=mock-user2");
        await user2Page.WaitForURLAsync(new Regex(@"^" + Regex.Escape(_factory.ServerAddress) + @"/?$"),
            new PageWaitForURLOptions { Timeout = 10000 });

        // 2. アイテムのオーナー(user1)としてログインする
        IPage ownerPage = await context.NewPageAsync();
        await ownerPage.GotoAsync($"{_factory.ServerAddress}/auth/callback?provider=Google&code=mock-user1");
        await ownerPage.WaitForURLAsync(new Regex(@"^" + Regex.Escape(_factory.ServerAddress) + @"/?$"),
            new PageWaitForURLOptions { Timeout = 10000 });
        ...
        // リクエストリストを探す
        ILocator requestList = ownerPage.Locator("div").Filter(new LocatorFilterOptions { HasTextString = "タグ付けリクエスト" })
            .First;

        // 「リジェクト」ボタンを探す
        ILocator rejectButton = requestList
            .Locator("button", new LocatorLocatorOptions { HasTextRegex = new Regex("リジェクト|Reject|却下") }).First;
        ...
        if (await rejectButton.IsVisibleAsync())
        {
            ...
            ILocator dialog = ownerPage.Locator(".mud-dialog");
            await dialog.WaitForAsync(new LocatorWaitForOptions { State = WaitForSelectorState.Visible });

            ILocator reasonInput = dialog.Locator("textarea");
            await reasonInput.FillAsync("今は不要です");

            ILocator submitButton = dialog.Locator("button").Filter(new LocatorFilterOptions { HasTextString = "却下する" })
                .First;
            ...
            ILocator requestCard = ownerPage.Locator(".mud-card-content")
                .Filter(new LocatorFilterOptions { HasTextString = "今は不要です" }).First;
