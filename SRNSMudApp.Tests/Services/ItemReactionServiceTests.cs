...
/// <summary>
/// <see cref="ItemReactionService" /> の単体テスト (MSSQL Testcontainers)。
/// </summary>
public class ItemReactionServiceTests : IAsyncLifetime
{
    ...
    private async Task<(ApplicationDbContext db, ItemReactionService sut, string userId, int goodTagId, int itemId, string tid)> CreateScopeAsync()
    {
        var tid = Guid.NewGuid().ToString("N")[..8];
        var db = new ApplicationDbContext(_sharedDb.Options);
        var sut = new ItemReactionService(new DbContextFactoryStub(_sharedDb.Options));
