...
customElements.define('passkey-submit', class extends HTMLElement {
    ...
    connectedCallback() {
        ...
        this.internals.form.addEventListener('submit', event => {
            if (!this._isSubmitting && event.submitter?.name === '__passkeySubmit') {
                event.preventDefault();
                this.obtainAndSubmitCredential().catch(console.error);
            }
        });

        this.tryAutoFillPasskey().catch(console.error);
    }
    ...
    async obtainAndSubmitCredential(useConditionalMediation = false) {
        ...
        // Instead of this.internals.form.submit() which bypasses Blazor's enhanced navigation
        // and form handling (resulting in missing _handler parameter),
        // we use requestSubmit() or manually inject the handler if needed.
        // Actually, since we want Blazor to handle it, requestSubmit() is the standard way.
        if (typeof this.internals.form.requestSubmit === 'function') {
            // Find the original submit button to pass as submitter if possible,
            // but passkey-submit itself is a form-associated custom element so we can try passing it.
            // Alternatively, just requestSubmit() to let Blazor intercept it.
            // Note: to prevent infinite loop of our own submit event listener, we should temporarily remove it or set a flag.
