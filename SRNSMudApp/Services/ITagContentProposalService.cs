...
/// <summary>
///     他ユーザーの Tag に対する「Content（説明文・内容）の編集提案」リクエストの申請、承認、却下、取り下げを管理するドメインサービス。
/// </summary>
public interface ITagContentProposalService
{
    /// <summary>
    ///     他ユーザーの Tag に対し、Content の編集提案リクエストを申請する。
    /// </summary>
    /// <param name="tagId">提案対象のタグID。</param>
    /// <param name="proposedContent">提案する新しい Content。</param>
    ...
    Task<Result<TagContentProposal>> ProposeContentAsync(
        int tagId,
    ...
    /// <summary>
    ///     タグ所有者が編集提案を承認する。
    ///     Tag の Content が提案内容に更新され、ベクトルの再生成が行われる。
    /// </summary>
    ...
    /// <summary>
    ///     指定されたタグに対する保留中（Proposed）の編集提案一覧を取得する。
    /// </summary>
    /// <param name="tagId">タグID。</param>
    /// <param name="cancellationToken">キャンセラレーショントークン。</param>
    /// <returns>保留中の提案一覧。</returns>
    Task<IReadOnlyList<TagContentProposal>> GetPendingProposalsForTagAsync(
        int tagId,
        CancellationToken cancellationToken = default);

    /// <summary>
    ///     指定された編集提案を関連エンティティ込みで取得する。
    /// </summary>
    /// <param name="proposalId">提案ID。</param>
    /// <param name="cancellationToken">キャンセラレーショントークン。</param>
    /// <returns>提案エンティティ（存在しない場合は null）。</returns>
    Task<TagContentProposal?> GetProposalByIdAsync(
        int proposalId,
        CancellationToken cancellationToken = default);
}